...
const BannerDisplay: React.FC<BannerDisplayProps> = ({
  ...
  return (
    <div
      className={`banner-container relative ${className}`}
      role="complementary"
    ...
    >
      ...
      {currentBanner.banner_type === "html" && currentBanner.html_content && (
        <div
          className="html-banner rounded-lg overflow-hidden"
      ...
      {currentBanner.banner_type === "text" && currentBanner.text_content && (
        <div
          className={`text-banner p-4 bg-gradient-to-r from-orange-50 to-red-50 border border-orange-200 rounded-lg shadow-sm hover:shadow-md transition-all ${
            currentBanner.click_url
        ...
        >
          <div className="flex items-start justify-between">
            ...
            {currentBanner.click_url && (
              <ExternalLink
                className="h-5 w-5 text-orange-600 flex-shrink-0 ml-3"
                aria-hidden="true"
              />
            )}
          </div>
