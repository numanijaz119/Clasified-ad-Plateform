...
export const useListingModal = () => {
  ...
  const { loading, error, fetchAdDetails, transformToModalListing, createFallbackModalListing } = useAdDetails();

  const handleListingClick = useCallback(async (basicAd: BasicAd) => {
    try {
