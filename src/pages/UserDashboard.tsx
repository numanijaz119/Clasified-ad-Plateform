...
const UserDashboard: React.FC = () => {
  ...
  const [userAds, setUserAds] = useState<UserAd[]>([
    ...
    },
    {
      ...
      status: "expired",
      description:
        'MacBook Pro 14" with M1 Pro chip. Barely used, includes original box.',
    },
  ]);
  ...
  const handlePromoteAd = (adId: number) => {
    setUserAds((ads) =>
      ads.map((ad) => (ad.id === adId ? { ...ad, featured: true } : ad))
    );
  };
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {userAds.map((ad) => (
                <tr key={ad.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {ad.views > 0
                      ? ((ad.inquiries / ad.views) * 100).toFixed(1)
                      : 0}
                    %
                  </td>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="p-6">
          ...
          {userAds.filter((ad) => !ad.featured && ad.status === "active")
            .length === 0 && (
            <div className="text-center py-8">
              <Star className="h-12 w-12 text-gray-400 mx-auto mb-4" />
              <h3 className="text-lg font-semibold text-gray-900 mb-2">
                No ads eligible for promotion
              </h3>
              <p className="text-gray-600">
                All your active ads are already featured or you don't have any
                active ads.
              </p>
            </div>
          )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
          <div className="border-b border-gray-200">
            <nav className="flex gap-x-8 flex-wrap px-6" aria-label="Tabs">
              {tabs.map((tab) => (
                <button
                  key={tab.id}
